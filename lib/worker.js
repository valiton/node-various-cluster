...
module.exports = Worker = (function() {
  ...
  _shutdown = function() {
    ...
    if (shutDowns === 0) {
      _log.call(this, 'notice', util.format('%s with pid %s is done with graceful shutdown, exit now', this.config.title, process.pid));
  ...
  _appendEvents = function() {
    ...
    process.on('uncaughtException', function(err) {
      if (_this.connected) {
        _log.call(_this, 'crit', util.format('%s with pid %s had uncaught exception: %s', _this.config.title, process.pid, err.stack));
